...
npm run trim-demo -- <file.json> [startDate] [endDate] [-o output.json] # Create lightweight demo dataset from full sleep data export
...
- **Demo data** — `src/demo-data.json` is a trimmed subset (~32KiB, no stage-level data) loaded via `data.loadDemoData()`; generated by `cli/trim-demo.ts` from a full export
...
| `src/data/useGoogleHealthData.ts`                | Data orchestrator: cache-first fetch, export, abort, demo data                       |
| `src/data/demo-data.json`                        | Bundled trimmed demo dataset (~32KiB) for Google verification                        |
| `src/components/EmptyState.tsx`                   | Shown when no data loaded; offers "Load demo data" button                            |
...
| `cli/trim-demo.ts`                               | CLI to create lightweight demo dataset from full sleep data export                    |
